/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MlmMatrixWhereInput } from "./MlmMatrixWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { MlmMatrixOrderByInput } from "./MlmMatrixOrderByInput";

@ArgsType()
class MlmMatrixFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => MlmMatrixWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => MlmMatrixWhereInput, { nullable: true })
  @Type(() => MlmMatrixWhereInput)
  where?: MlmMatrixWhereInput;

  @ApiProperty({
    required: false,
    type: [MlmMatrixOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [MlmMatrixOrderByInput], { nullable: true })
  @Type(() => MlmMatrixOrderByInput)
  orderBy?: Array<MlmMatrixOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { MlmMatrixFindManyArgs as MlmMatrixFindManyArgs };
