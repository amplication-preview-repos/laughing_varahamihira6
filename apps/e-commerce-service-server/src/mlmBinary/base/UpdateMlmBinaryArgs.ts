/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MlmBinaryWhereUniqueInput } from "./MlmBinaryWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { MlmBinaryUpdateInput } from "./MlmBinaryUpdateInput";

@ArgsType()
class UpdateMlmBinaryArgs {
  @ApiProperty({
    required: true,
    type: () => MlmBinaryWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => MlmBinaryWhereUniqueInput)
  @Field(() => MlmBinaryWhereUniqueInput, { nullable: false })
  where!: MlmBinaryWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => MlmBinaryUpdateInput,
  })
  @ValidateNested()
  @Type(() => MlmBinaryUpdateInput)
  @Field(() => MlmBinaryUpdateInput, { nullable: false })
  data!: MlmBinaryUpdateInput;
}

export { UpdateMlmBinaryArgs as UpdateMlmBinaryArgs };
